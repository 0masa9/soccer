...
export function AddTeamForm({
  ...
  const [state, formAction] = useActionState(addLeagueTeam, initialState);

  return (
    <form action={formAction} className="space-y-3">
      <input type="hidden" name="league_id" value={leagueId} />
      <div className="space-y-2">
        <Label htmlFor="team-select">追加するチーム</Label>
        <Select id="team-select" name="team_id" defaultValue="" required>
...
export function MatchCreateForm({
  ...
  const [state, formAction] = useActionState(createLeagueMatch, initialState);
    ...
    <form action={formAction} className="space-y-3">
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <div className="space-y-2">
          ...
          <Select id="home-team" name="home_team_id" defaultValue="" required>
            <option value="" disabled>
              チームを選択
            </option>
            {teams.map((team) => (
        ...
        <div className="space-y-2">
          ...
          <Select id="away-team" name="away_team_id" defaultValue="" required>
            <option value="" disabled>
              チームを選択
            </option>
            {teams.map((team) => (
        ...
        <div className="space-y-2">
          <Label htmlFor="kickoff-date">キックオフ日（任意）</Label>
          <Input id="kickoff-date" type="date" name="kickoff_date" />
        </div>
      </div>
      <FormError message={!state.ok ? state.message : undefined} />
      <SubmitButton variant="outline">試合を追加</SubmitButton>
    </form>
  );
}

export function MatchResultForm({ match }: { match: Match }) {
  const [state, formAction] = useActionState(updateLeagueMatch, initialState);
